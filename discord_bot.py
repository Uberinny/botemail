...
@app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
@app_commands.user_install()
@app_commands.describe(initials='Staff initials', amount='Debt amount')
async def paid(interaction: discord.Interaction, initials: str, amount: float):
    
    from datetime import datetime
    import random
    order_id = random.randint(100, 999)
    # Add $1 fee to debt
    amount_with_fee = amount + 1.00
    order = {
        'order_id': order_id,
    ...
    save_data()
    await interaction.response.send_message(f"Recorded: Order #{order_id} | {initials.upper()} | Debt: ${amount:.2f} + $1.00 fee = ${amount_with_fee:.2f}")
...
async def staffdash(interaction: discord.Interaction):
    ...
    for initials in sorted(staff_orders.keys()):
        msg += f"👤 === {initials} ===\n"
        msg += f"{'Order #':<10} {'Debt':<15} {'Date/Time'}\n"
        msg += "-" * 45 + "\n"
        ...
        for order in staff_orders[initials]:
            ...
            edited = order.get('edited', False)
            timestamp = order['timestamp'].split('T')[0] + ' ' + order['timestamp'].split('T')[1][:8]
            edited_marker = " (edited)" if edited else ""
            msg += f"{order_id:<10} ${amount:<9.2f}{edited_marker:<6} {timestamp}\n"
            staff_debt += amount
    ...
    for order in recent_orders:
        ...
        timestamp = order['timestamp'].split('T')[0] + ' ' + order['timestamp'].split('T')[1][:8]
        edited_marker = " (edited)" if edited else ""
        msg += f"#{order_id} | {initials} | Debt: ${amount:.2f}{edited_marker} | {timestamp}\n"
...
class OrderSelectView(View):
    def __init__(self, orders):
        ...
        # Create select menu options with unique values using index
        options = []
        for idx, order in enumerate(orders):
            ...
            options.append(discord.SelectOption(label=label, value=f"{idx}_{order_id}"))
    ...
    async def select_callback(self, interaction: discord.Interaction):
        # Parse the value to get index and order_id
        value_parts = self.select_menu.values[0].split('_')
        idx = int(value_parts[0])
        selected_order_id = int(value_parts[1])
        
        # Get the order from our stored list using index
        if idx < len(self.orders):
            selected_order = self.orders[idx]
            view = OrderActionView(selected_order_id, selected_order)
            await interaction.response.send_message(
                f"Selected Order #{selected_order_id} | {selected_order['initials']} | Debt: ${selected_order.get('amount', 0):.2f}",
                view=view,
                ephemeral=True
            )
        else:
        ...
        try:
            ...
            for order in paid_orders:
                if order.get('order_id') == self.order_id:
                    ...
                    order['edited'] = True  # Mark as edited
                    save_data()
                    await interaction.response.send_message(
                        f"✅ Updated order #{self.order_id} debt: ${old_amount:.2f} → ${new_amount:.2f} (marked as edited)"
                    )
                    return
            await interaction.response.send_message("❌ Order not found.")
        except ValueError:
