...
@app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
@app_commands.user_install()
@app_commands.describe(initials='Staff initials', amount='Profit amount')
async def paid(interaction: discord.Interaction, initials: str, amount: float):
    ...
    save_data()
    await interaction.response.send_message(f"Recorded: {initials.upper()} - ${amount:.2f}")
...
async def staffdash(interaction: discord.Interaction):
    ...
    for initials in sorted(staff_orders.keys()):
        ...
        for order in staff_orders[initials]:
            ...
            msg += f"💵 ${amount:<9.2f} {timestamp}\n"
            staff_total += amount
        
        msg += "-" * 35 + "\n"
        msg += f"🔴 Debt: ${staff_total:.2f}\n\n"
        grand_total += staff_total
    
    msg += "=" * 35 + "\n"
    msg += f"💰 AMOUNT DUE: ${grand_total:.2f}"
    
    await interaction.response.send_message(f"```\n{msg}\n```")

@tree.command(name='listorder', description='List all orders with options to delete or edit')
@app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
@app_commands.user_install()
async def listorder(interaction: discord.Interaction):
    if not paid_orders:
        return await interaction.response.send_message('No orders to display.')
    
    # Group orders by staff
    staff_orders = {}
    order_indices = {}  # Track global index for each order
    global_idx = 0
    ...
    for order in paid_orders:
        ...
        if initials not in staff_orders:
            staff_orders[initials] = []
            order_indices[initials] = []
        staff_orders[initials].append(order)
        order_indices[initials].append(global_idx)
        global_idx += 1
    
    # Build order list separated by staff
    msg = "📋 All Orders\n\n"
    
    for initials in sorted(staff_orders.keys()):
        msg += f"👤 **{initials}**\n"
        for i, order in enumerate(staff_orders[initials]):
            order_num = order_indices[initials][i] + 1
            amount = order['amount']
            timestamp = order['timestamp'].split('T')[0] + ' ' + order['timestamp'].split('T')[1][:8]
            msg += f"**{order_num}.** ${amount:.2f} - {timestamp}\n"
        msg += "\n"
...
class OrderListView(View):
    ...
    async def delete_button(self, interaction: discord.Interaction, button: Button):
        await interaction.response.send_modal(DeleteOrderModal())
    
    @discord.ui.button(label="Change Amount", style=discord.ButtonStyle.primary, emoji="✏️")
    async def edit_button(self, interaction: discord.Interaction, button: Button):
        await interaction.response.send_modal(ChangeAmountModal())

class DeleteOrderModal(discord.ui.Modal, title="Delete Order"):
    order_number = discord.ui.TextInput(
        label="Order Number",
        placeholder="Enter order number to delete",
        required=True,
        max_length=5
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            idx = int(self.order_number.value) - 1
            if 0 <= idx < len(paid_orders):
                deleted_order = paid_orders.pop(idx)
                save_data()
                await interaction.response.send_message(
                    f"✅ Deleted order: {deleted_order['initials']} - ${deleted_order['amount']:.2f}"
                )
            else:
                await interaction.response.send_message("❌ Invalid order number.")
        except ValueError:
            await interaction.response.send_message("❌ Please enter a valid number.")

class ChangeAmountModal(discord.ui.Modal, title="Change Order Amount"):
    order_number = discord.ui.TextInput(
        label="Order Number",
    ...
    new_amount = discord.ui.TextInput(
        label="New Amount",
        placeholder="Enter new amount (e.g., 25.50)",
        required=True,
        max_length=10
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if 0 <= idx < len(paid_orders):
                old_amount = paid_orders[idx]['amount']
                paid_orders[idx]['amount'] = amount
                save_data()
                await interaction.response.send_message(
                    f"✅ Changed order {idx + 1}: ${old_amount:.2f} → ${amount:.2f}"
                )
            else:
                await interaction.response.send_message("❌ Invalid order number.")
        except ValueError:
